 ...
 */
@discardableResult
public func showCustomAlert(in viewController: UIViewController, withTitle title: String, andMessage message: String, completion: (()->Void)? = nil, buttons: (name: String, handler: ((UIAlertAction)->Void)?)...) -> UIAlertController {
...
/**
 ...
 - Parameters:
    ...
    - message: The message thatt will be shoed in the alert
    - completion: A closure that will be run when the alert has been full showed. Could be nil.
    - okButtonHandler: A closuer that will be run when OK button is clicked. Could be nil.
 */
@discardableResult
public func showOkAlert(in viewController: UIViewController, withTitle title: String, andMessage message: String, completion: (()-> Void)? = nil, okButtonHandler: ((UIAlertAction)->Void)? = nil) -> UIAlertController {
    let alert = UIAlertController(title: title, message: message, preferredStyle: .alert)
    alert.addAction(UIAlertAction(title: "Ok", style: .default, handler: nil))
    viewController.present(viewController, animated: true, completion: completion)
